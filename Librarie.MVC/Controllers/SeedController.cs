...
namespace Library.MVC.Controllers
{
    public class SeedController : Controller
    {
        ...
        public async Task<IActionResult> SeedLibrary()
        {
            ...
            // 5. Setup Loan Generator (Base properties only)
            ...
            // 6. MANUALLY ASSIGN STATUS (5 Returned, 5 Active, 5 Overdue)
            for (int i = 0; i < loans.Count; i++)
            {
                if (i < 5) // FIRST 5: RETURNED
                {
                ...
                else if (i < 10) // NEXT 5: ACTIVE (IN-TIME)
                {
                    loans[i].LoanDate = DateTime.Now.AddDays(-5);
                    loans[i].DueDate = loans[i].LoanDate.AddDays(14); // Due in future
                    loans[i].ReturnedDate = null;
                    loans[i].IsReturned = false;
                }
                else // REMAINING 5: ACTIVE AND OVERDUE
                {
                    loans[i].LoanDate = DateTime.Now.AddDays(-30);
                    loans[i].DueDate = loans[i].LoanDate.AddDays(14); // Due 16 days ago
                    loans[i].ReturnedDate = null;
                    loans[i].IsReturned = false;
                }
            }

            // 7. THE SYNC WORKFLOW: Update Book Availability
